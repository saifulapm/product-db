...
<div class="flex items-center justify-between mb-6 flex-wrap gap-4 flex-shrink-0">
    <button 
        wire:click="previous" 
...
<!-- Week View -->
@if($viewType === 'week')
<div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border-2 border-gray-300 dark:border-gray-600 overflow-hidden" style="{{ isset($isFullScreen) && $isFullScreen ? 'flex: 1 1 0; min-height: 0; height: calc(100vh - 200px);' : 'flex: 1 1 0; min-height: 0;' }}">
    <div class="grid grid-cols-7 gap-0" style="height: 100%;">
        @foreach($this->getWeekDays() as $date)
            ...
            <div class="flex flex-col h-full {{ !$isLast ? 'border-r border-gray-300 dark:border-gray-600' : '' }}" style="{{ !$isLast ? 'border-right: 1px solid rgb(209 213 219) !important;' : '' }}">
                ...
                <!-- Tasks Column -->
                <div class="p-3 flex-1 bg-white dark:bg-gray-800 overflow-y-auto min-h-0">
...
@if($viewType === '3day')
<div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border-2 border-gray-300 dark:border-gray-600 overflow-hidden" style="{{ isset($isFullScreen) && $isFullScreen ? 'flex: 1 1 0; min-height: 0; height: calc(100vh - 200px);' : 'flex: 1 1 0; min-height: 0;' }}">
    <div class="grid grid-cols-3 gap-0" style="display: grid; grid-template-columns: repeat(3, 1fr); height: 100%;">
...
<div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border-2 border-gray-300 dark:border-gray-600 overflow-hidden" style="{{ isset($isFullScreen) && $isFullScreen ? 'flex: 1 1 0; min-height: 0; height: calc(100vh - 200px);' : 'flex: 1 1 0; min-height: 0;' }}">
    ...
    <div class="flex flex-col h-full">
        ...
        <!-- Tasks Column -->
        <div class="p-6 flex-1 bg-white dark:bg-gray-800 overflow-y-auto min-h-0">
            @include('filament.resources.task-resource.pages.partials.task-list', ['tasks' => $tasks])
        </div>
    </div>
</div>
